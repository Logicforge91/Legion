...
export default function ProjectsSection() {
  ...
  return <section id="projects" className="projects-section systems-showcase" aria-labelledby="projectsTitle"><div className="site-container">
    ...
    <div className={`projects-grid systems-grid${visibleProjects.length === 1 ? ' single-result' : ''}`}>{visibleProjects.map((project) => { const systemNumber = systemNumbers.get(project.slug); return <Reveal key={project.title} className={`system-card-shell system-card-${systemNumber}`} data-system={project.filters[0]}><Spotlight className="project-card system-card"><span className="system-card-glow" aria-hidden="true" /><div className="project-top"><div className="system-icon"><i className={`bi ${project.icon}`} aria-hidden="true" /></div><div className="system-card-meta"><span>{project.category}</span><small>Case study / {String(systemNumber).padStart(2, '0')}</small></div></div><div className="system-card-copy"><h3>{project.title}</h3><p>{project.text}</p></div><div className="system-scenario" aria-label={`Representative ${project.category} runtime flow`}><div className="system-scenario-head"><span><i aria-hidden="true" />Representative flow</span><small>illustrative</small></div><code>{project.scenario.request}</code><div className="system-scenario-route"><span>{project.scenario.event}</span><i className="bi bi-arrow-right" aria-hidden="true" /><strong>{project.scenario.result}</strong></div></div><ul className="system-points">{project.points.map((point) => <li key={point}><i className="bi bi-check2" aria-hidden="true" />{point}</li>)}</ul><div className="system-card-footer"><div className="tag-row">{project.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><button className="project-details-button" type="button" onPointerEnter={loadProjectDialog} onFocus={loadProjectDialog} onClick={() => openProject(project)}><span>Open system</span><i className="bi bi-arrow-up-right" aria-hidden="true" /></button></div></Spotlight></Reveal>; })}</div>
